...
void ObjFile::printSet(OGLE::ElementSetPtr set)
{
	...
	else if(set->mode == GL_TRIANGLE_STRIP)
	{
		...
		for(i = 0; i < set->elements.size(); i++)
		{
			...
			if(i >= 2)
			{
				printFace(face,	OGLE::config.flipPolyStrips	&& (flip_flag = (flip_flag + 1) % 2) );
...
void ObjFile::printFace(FacePtr face, bool flip)
{
	...
	fprintf(f, "f ");
	for(int i = (flip ? face->elements.size() - 1 : 0);i < face->elements.size() && i >= 0; i += (flip ? -1 : 1))
	{
...
void ObjFile::printVertex(OGLE::VertexPtr v, const char *typeStr, int n)
{
	...
	if (n >= 1) {if (!isNumber(v->x)) v->x = 0; fprintf(f, " %e", v->x);}
	if (n >= 2) { if (!isNumber(v->y)) v->y = 0; fprintf(f, " %e", v->y); }
	if (n >= 3) { if (!isNumber(v->z)) v->z = 0; fprintf(f, " %e", v->z); }
