...
void  OGLE::glVertexfv(GLfloat *V, GLsizei n)
{
	...
	if(currSet)
	{
		currSet->addElement(new OGLE::Element(new OGLE::Vertex(V, n), 
								...
								(OGLE::config.captureColors ? currColor : 0)
								)
							);
	}
}
...
void OGLE::glColorfv(GLfloat *V, GLsizei n)
{
	if (OGLE::config.captureColors)
	{
		currColor = new OGLE::Vertex(V, n);
		currColor->w = 1;
	}
}
...
void OGLE::glDrawRangeElements(GLenum mode, GLuint start, GLuint end, GLsizei count,GLenum type, const GLvoid *indices)
{
...
void OGLE::glDrawElements (GLenum mode, GLsizei count, GLenum type, const GLvoid *indices)
{
	...
	const GLbyte *p = vArray.data;
	p = getBufferedArray(p);
	GLuint buffIndex = getBufferIndex(GL_ARRAY_BUFFER);	
	...
	for(int i = 0; i < count; i ++)
	{
		...
		//--------------------调试----------------------------
		GLsizei size = glTypeSize(vArray.type);
		if (buff && buff->size < index * (vArray.stride ? vArray.stride : (vArray.size * size)))
		{
			GLuint buffIndex_E = getBufferIndex(GL_ELEMENT_ARRAY_BUFFER);	 
			BufferPtr buff_E = buffers[buffIndex_E];
			continue;
		}
...
void OGLE::glArrayElement(GLint i)
{
	...
	if (OGLE::config.captureColors && cArray.enabled)
	{
		if (derefClientArray(&cArray, V, i))
		{
			glColorfv(V, cArray.size);
		}
	}
...
void OGLE::glEnableClientState (GLenum array)
{
  switch(array)
  {
	  ...
	  case GL_COLOR_ARRAY:  cArray.enabled = true; break;
...
void OGLE::glColorPointer(GLenum type, GLsizei stride, const GLvoid *pointer)
{
	cArray.size = 3;
	cArray.type = type;
	cArray.stride = stride;
	cArray.data = (const GLbyte *)pointer;
...
void OGLE::glInterleavedArrays(GLenum format, GLsizei stride, const GLvoid *pointer)
{
	...
	int f = sizeof(GL_FLOAT), c = 4 * sizeof(GL_UNSIGNED_BYTE);
	...
	if (ec) {
		this->glEnableClientState(GL_COLOR_ARRAY);
		//这个第一个参数是应该是3,还是4？
		this->glColorPointer(/*4, */GL_FLOAT, str, ((GLbyte *)pointer) + pc);
	}
	else
...
void OGLE::initFunctions()
{
	...
	if(oglVersion >= 1.5f || callBacks->IsGLExtensionSupported("GL_ARB_vertex_buffer_object"))
	{
