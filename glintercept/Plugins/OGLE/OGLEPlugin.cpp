...
void OGLEPlugin::ProcessConfigData(ConfigParser *parser)
{
  ...
  testToken = parser->GetToken("CaptureColors");

  if (testToken)
  {
	  testToken->Get(OGLE::config.captureColors);
	  fprintf(OGLE::LOG, "CAPTURE COLORS: %d\n", OGLE::config.captureColors);
  }
	...
	isRecording(0)
{
	...
	gliCallBacks->RegisterGLFunction("glColorPointer");
	...
	/*需要完善法线和颜色相关的函数！！！，还有很多没有添加的函数*/
	gliCallBacks->RegisterGLFunction("glColor3fv");
	gliCallBacks->RegisterGLFunction("glColor3f");
	gliCallBacks->RegisterGLFunction("glColor4ub");
	gliCallBacks->RegisterGLFunction("glColor4f");
	gliCallBacks->RegisterGLFunction("glMaterialfv");

   //Get calls that are even outside contexts  
	gliCallBacks->SetContextFunctionCalls(true);

	//Parse the config file
	//先解析插件文件夹下的config.ini
	...
	//再解析主配置文件gliConfig.ini 中有关插件的设置
...
void OGLEPlugin::GLFunctionPre (uint updateID, const char *funcName, uint funcIndex, const FunctionArgs & args )
{
	...
	if(isRecording || OGLE_BIND_BUFFERS_ALL_FRAMES)
	{
		...
		if(strcmp(funcName, "glBindBuffer") == 0 || strcmp(funcName, "glBindBufferARB") == 0)
		{
			GLenum  target; _args.Get(target);
			GLuint  buffer; _args.Get(buffer);
			ogle->glBindBuffer(target , buffer);
		}
		else if(strcmp(funcName, "glBufferData") == 0 || strcmp(funcName, "glBufferDataARB") == 0)
		{
			GLenum  target; _args.Get(target);
			...
			fprintf(OGLE::LOG, "PRE FUNCTION :%04X  %d  %04X  %d  %s\n", target,size,usage, data,"glBufferData");
		}
		...
		if(OGLE_CAPTURE_BUFFERS_ALL_FRAMES)
		{
			...
			if(strcmp(funcName, "glBufferSubData") == 0	|| strcmp(funcName, "glBufferSubDataARB") == 0)
			{
				GLenum  target; _args.Get(target);
			...
			else if(strcmp(funcName, "glMapBuffer") == 0 || strcmp(funcName, "glMapBufferARB") == 0)
			{
				GLenum  target; _args.Get(target);
				GLenum  access; _args.Get(access);
				ogle->glMapBuffer(target , access);
			}
			else if(strcmp(funcName, "glUnmapBuffer") == 0 || strcmp(funcName, "glUnmapBufferARB") == 0)
			{
				GLenum  target; _args.Get(target);
	...
	else if(strcmp(funcName, "glDrawRangeElements") == 0 || strcmp(funcName, "glDrawRangeElementsEXT") == 0)
	{
		GLenum  mode; _args.Get(mode);
	...
	else if (strcmp(funcName, "glColor3fv") == 0)
	{
		//GLfloat *V; _args.Get(V);
		void *V; _args.Get(V);
		ogle->glColorfv((GLfloat *)V, 3);
	}
	else if (strcmp(funcName, "glColor3f") == 0)
	{
		GLfloat V[3]; _args.Get(*V); _args.Get(*(V + 1)); _args.Get(*(V + 2));
		ogle->glColorfv(V, 3);
	}
	else if (strcmp(funcName, "glColor4ub") == 0)
	{
		GLubyte tmp;
		GLubyte V[3]; _args.Get(*V); _args.Get(*(V + 1)); _args.Get(*(V + 2));_args.Get(tmp);
		GLfloat ff[3];
		ff[0] = V[0]; ff[1] = V[1]; ff[2] = V[2];
		ff[0] /= 255.0f; ff[1] /= 255.0f; ff[2] /= 255.0f;
		ogle->glColorfv(ff, 3);
	}
	else if (strcmp(funcName, "glColor4f") == 0)
	{
		GLfloat V[3]; _args.Get(*V); _args.Get(*(V + 1)); _args.Get(*(V + 2));
		GLfloat tmp;
		_args.Get(tmp);
	...
	else if (strcmp(funcName, "glMaterialfv") == 0)
	{
		GLenum face; _args.Get(face);
		GLenum pname; _args.Get(pname);
		GLvoid *params; _args.Get(params);
		GLfloat *ptr = (GLfloat *)params;
		if (pname == GL_AMBIENT_AND_DIFFUSE)
		{
			//ogle->glColorfv(ptr, 3);
		}		
	}
	...
	else if(strcmp(funcName, "glClientActiveTexture") == 0 || strcmp(funcName, "glClientActiveTextureARB") == 0)
	{
	...
	else if (strcmp(funcName, "glColorPointer") == 0)
	{
		GLenum type; _args.Get(type);
		GLsizei stride; _args.Get(stride);
		GLvoid *pointer; _args.Get(pointer);
		ogle->glColorPointer(type, stride, pointer);
	}
...
void OGLEPlugin::GLFunctionPost(uint updateID, const char *funcName, uint funcIndex, const FunctionRetValue & retVal)
{
	...
	{
		if(strcmp(funcName, "glMapBuffer") == 0 || strcmp(funcName, "glMapBufferARB") == 0)
		{
