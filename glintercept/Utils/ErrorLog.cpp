...
std::string ErrorLog::LOG_DEFAULT_NAME = "Hook.text";

#define MAX_DEBUG_BUFFER 1024

static ErrorLog * glogalInstanceLog = nullptr;

ErrorLog & ErrorLog::GetInstance()
{
	if (!glogalInstanceLog)
	{
		glogalInstanceLog = new ErrorLog(LOG_DEFAULT_NAME.c_str());
	}

	return *glogalInstanceLog;
}
